...
import dialogue.voiture.PVoitures;
...
public class FAccueil extends JFrame {
	...
	private final Action actionVoitures = new ActionVoitures();
	...
	public FAccueil(Connexion connexion) {
		...
		mntmVoitures.setAction(actionVoitures);
		mnVues.add(mntmVoitures);
		UI.habiller(mntmVoitures);
		...
		JButton btnVoitures = new JButton();
		btnVoitures.setIcon(new ImageIcon(FAccueil.class.getResource("/images/accueil/People-128-actif.png")));
		UI.deshabillerBouton(btnVoitures, "accueil", "People", 128);
		...
		btnVoitures.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {
				lblInfos.setText("Voitures");
			}
		...
		btnVoitures.setCursor(new Cursor(Cursor.HAND_CURSOR));			
		panel_principal.add(btnVoitures, "cell 0 1,alignx center");
...
//	    laFenetre.setVisible(true);
	...
	protected void afficherStats() {
	    FTableauBord tableau = new FTableauBord(this, connexion);
...
//	    laFenetre.setVisible(true);
	...
	protected void afficherVoitures() {
		JDialog dialog = new JDialog(this);
		PVoitures voitures = new PVoitures();
		voitures.setConnexion(connexion);
		voitures.setFenetre(dialog);
		dialog.setContentPane(voitures);
		dialog.setIconImage(UI.getLogo());
		dialog.setTitle("Gestion des Voitures");
	...
	private class ActionVoitures extends AbstractAction {
		private static final long serialVersionUID = 1L;

		public ActionVoitures() {
			putValue(NAME, "Voitures");
			putValue(SHORT_DESCRIPTION, "Afficher la liste des voitures");

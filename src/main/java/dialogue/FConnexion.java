...
public class FConnexion extends JFrame {
	...
	private void createContents() {
		...
		setTitle(Messages.getString("FConnexion.0"));
		
		JLabel lblTitre = new JLabel(Messages.getString("FConnexion.4"));
		...
		JLabel lblUtilisateur = new JLabel(Messages.getString("FConnexion.7"));
		...
		txtSaisieUtilisateur.setText(Messages.getString("FConnexion.login"));
		...
		JLabel lblMotDePasse = new JLabel(Messages.getString("FConnexion.12"));
		...
		JButton btnParamtres = new JButton(Messages.getString("FConnexion.17"));
		...
		JButton btnQuitter = new JButton(Messages.getString("FConnexion.23"));
	...
	private void valider() {
		...
		new SwingWorker<Boolean, Void>() {
			...
			protected Boolean doInBackground() throws Exception {
				...
				return connexion.controle(leNom, leMotDePasse);
			}
			...
			protected void done() {
				...
				try {
					...
					if (resultat) {						
						...
						.showMessageDialog(
								btnValider,
								Messages.getString("FConnexion.1"), //$NON-NLS-1$
								Messages.getString("FConnexion.32"), JOptionPane.ERROR_MESSAGE); //$NON-NLS-1$
					}
					...
					.showMessageDialog(
							btnValider,
							Messages.getString("FConnexion.2"), //$NON-NLS-1$
							Messages.getString("FConnexion.32"), JOptionPane.ERROR_MESSAGE); //$NON-NLS-1$			
				}
			}
		}.execute();
	...
	private class ActionValider extends AbstractAction {
		...
		public ActionValider() {
			putValue(NAME, Messages.getString("FConnexion.20"));
